...
from parameters import N, beta, foodXvertex, food_condition, width, height, mov_matrix, rand_matrix, Jij, Theta, tau
...
class Model(Model):
    ...
    def __init__(self, beta = beta, Theta = Theta, Jij = Jij, N = N, 
        ...
        self.matrices = {'scout': mov_matrix, 'recruit': rand_matrix} 
        self.gains = {'scout': 0.8, 'recruit': 0.1}
        ...
        # states & rates
        self.states = {'beta': list(self.agents.values())}
        self.activity = {True: [], False: list(self.agents.values())}
        self.S = np.array([self.N])
        self.rates = np.array([beta])

  
        # Init first active agent
        self.agents[0].Si = 1 # np.random.uniform(0.0, 1.0)
...
#    'N': [], 'pos': [], 'food_target': [], 'id_out': []}
    ...
    def step(self, tmax):
        ...
        while self.time < tmax:
            ...
            # process = np.random.choice(['beta', 'gamma'], p = self.r_norm)
    ...
    def collect_data(self):
        ...
        i = self.agents[self.sampled_agent[-1]]
        xy = self.xy[i.pos]
        self.data['t'].append(self.time)
        self.data['id'].append(self.sampled_agent[-1])
        self.data['interaction'].append(i.last_interaction)
        self.data['node'].append(str(i.pos))
        self.data['x'].append(xy[0])
        self.data['y'].append(xy[1])
        self.data['movement'].append(i.behavior_tag + '_'+ i.movement)
    ...
    #     l = len(self.activity[True])
    #     if l:
    #         for i in self.activity[True]:
    #             node.append(str(i.pos))
    #             xy = self.xy[i.pos]
    #             x.append(xy[0])
    #             y.append(xy[1])
    #             target.append(i.behavior_tag + '_'+ i.movement)
    #             # target += int(hasattr(i, 'target') and i.target in self.food_coords)
    #             # Si_out += str(i.Si) + ','
    #             interaction.append(i.last_interaction)
    ...
    #         self.data['interaction'].extend(interaction)
    ...
    def init_agents(self, **kwargs):  
        ...
        ''' GAIN '''
        # determinist
        g = [self.gains['scout'] if b == 'scout' else self.gains['recruit'] for b in behav]
